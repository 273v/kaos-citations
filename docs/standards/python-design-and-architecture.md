...
These standards apply to Python code in `kaos-citations`.

`kaos-citations` publishes the `kaos_citations` import package. This package publishes CLI entry point(s): `kaos-citations-serve`.

## Package Shape

- Keep the import package name aligned with the distribution name:
  `kaos-citations` publishes import package `kaos_citations`.
- Declare the public API in `kaos_citations.__all__` when the package has
...
- Names exported from `kaos_citations.__all__`.
...
- Put optional integrations in named extras. Public extras currently declared: `legal`.
